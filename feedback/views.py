...
def d_admin_feedback_detail(request,id):
    ...
    if feedback_status:
        dosya_uzantısı = str(feedback_status.history_file).endswith(".txt")
    else:
        dosya_uzantısı = False
...
def d_admin_cozuldu(request):
    ...
    department = d_user.user_department
    feedbacks_list = Feedback.objects.filter(IdCompany=company_id,IdFeedbackStatus__name="Çözüldü",IdDepartment__name=department).order_by('-FeedbackOpenDate')
...
@login_required(login_url="user:login")
def d_admin_files_read(request,id):
    ...
    return render(request,"department_admin/d_admin_files_read.html",context)
...
def admin_all_feedback(request):
    ...
    return render(request,"admin/all_feedback.html",context)
...
@login_required(login_url="user:login")
def admin_files_read(request,id):
    feedback_status = FeedbackStatusHistory.objects.filter(id=id).first()
    liste= list()
    with open(feedback_status.history_file.path,"r",encoding="utf-8") as file:
        for f in file:
    ...
    return render(request,"admin/admin_files_read.html",context)
...
def admin_register(request):
    ...
    if user_add.is_valid():
        ...
        if user_add_profile.user_role.id != 2 :
            if user_add_profile.user_role.id != 3:
                messages.info(request,"Admin Kaydı Başarısız!' ")
                messages.info(request,"Rolü 'Hastane Admini' veya 'Departman Admini' Olmalıdır!' ")
        ...
        if user_name:
            messages.info(request,"Admin Kaydı Başarısız!' ")
            messages.info(request,"Önceden Kullanılmış Kullanıcı Adıdır!")
            return redirect("feedback:admin_register")
...
def admin_register_personel(request):
    ...
    if user_add.is_valid():
        user_add_profile = user_add.save(commit=False)
        if user_add_profile.user_role.id == 1:
            messages.info(request,"Rolü 'Admin'-'Hastane Admini'-'Departman Admini' Olamaz!")
